...
func HandleBasicAuthReq(req *http.Request, ctx *goproxy.ProxyCtx, f func(user, passwd string, ctx *goproxy.ProxyCtx) bool) bool {
	...
	return f(userpass[0], userpass[1], ctx)
}
...
func Basic(realm string, f func(user, passwd string, ctx *goproxy.ProxyCtx) bool) goproxy.ReqHandler {
	return goproxy.FuncReqHandler(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
		if !HandleBasicAuthReq(req, ctx, f) {
...
//
// You probably want to use auth.ProxyBasic(proxy) to enable authentication for all proxy activities
func BasicConnect(realm string, f func(user, passwd string, ctx *goproxy.ProxyCtx) bool) goproxy.HttpsHandler {
	return goproxy.FuncHttpsHandler(func(host string, ctx *goproxy.ProxyCtx) (*goproxy.ConnectAction, string) {
		if !HandleBasicAuthReq(ctx.Req, ctx, f) {
...
func ProxyBasic(proxy *goproxy.ProxyHttpServer, realm string, f func(user, passwd string, ctx *goproxy.ProxyCtx) bool) {
